...
!
!    Forward-Backward 3D complex transform for double precision data.
!
!****************************************************************************/
...
int main(int argc, char *argv[])
{
    ...
    if (0 == comm_rank) {
        printf("Number of MPI processes = %d, data cube dimensions = %d x %d x %d\n", 
               comm_size, (int)n0, (int)n1, (int)n2);
        printf("Usage of MPI FFTW for Complex Multi-dimensional Transforms\n");
    }

    size = fftw_mpi_local_size_3d(n0, n1, n2, MPI_COMM_WORLD, &local_n0, &local_0_start);
    ...
    srand(1);
    for (i=0; i<2*local_n0*n1*n2; i++)
        REAL_PTR(src)[i]=(double)rand()/RAND_MAX;

    plan = fftw_mpi_plan_dft_3d(n0, n1, n2, src, dst, MPI_COMM_WORLD, FFTW_FORWARD, FFTW_ESTIMATE);
    if (NULL == plan) {
        ret = 1;
    ...
    plan = fftw_mpi_plan_dft_3d(n0, n1, n2, dst, src, MPI_COMM_WORLD, FFTW_BACKWARD, FFTW_ESTIMATE);
    if (NULL == plan) {
        ret = 1;
        printf("Failed to create plan for forward FFT\n");
        goto cleanup;
