...
%possibility to run in parallel (for HPC)
...
% Summarize pairwise screening results and analyse:
    ...
    h = PCA_pertub_screening(Summary_screening,MyCombi)
...
%%
    ...
    if ~isequal(attr_init,zupdates_AC(per))
        [attrcnt,~,stateruns] = ssattr(per, runs, All_attractors, fixedtime,nodes,t);
